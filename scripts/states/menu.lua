...
function onCreate()
    ...
    if getDataFromSave('corruptMenu', 'seenIfNotice') == nil then
        setDataFromSave('corruptMenu', 'seenIfNotice', false)
    end

    if getDataFromSave('corruptMenu', 'seenSidestoryNotice') == nil then
        setDataFromSave('corruptMenu', 'seenSidestoryNotice', false)
    end
...
function updateMusic(which)
    playMusic('menu'..which, 0, true)
    runHaxeCode("FlxG.sound.music.fadeIn(1, 0, 1);")
end
...
function inputShit()
    ...
    if getProperty('controls.ACCEPT') then
        ...
        elseif curMenu == 'freeplay' then
            playSound('confirmMenu')
            canSelect = false
            setVar('settingStory', false)
            openCustomSubstate('diffSelector')
...
function startStoryMode()
    ...
    runTimer('play storymode', 5)
end

function startIf()
    startTween('initialTween', 'camGame', {zoom = 0.8}, 1.2, {ease = 'backIn', onComplete = 'storyTweenZoom'})

    runHaxeCode("FlxG.sound.music.fadeOut(1,0);")
...
function startSong()
    ...
    runTimer('transition', 0.75)
    runTimer('cover black', 1.7)
    runTimer('play freeplay song', 2.4)
end
...
function onConfirm() -- diff selector thing
...
function addMiddleShit()
    ...
    scaleObject('layer3', 1.3, 1.3, false)
    ...
    scaleObject('blackFade', 1.4, 1.3, false)

    makeLuaSprite('fullBlack', path..'mainmenu/fullBlack', -6, 1841)
    addLuaSprite('fullBlack', true)
...
function onTimerCompleted(tag)
    ...
    if tag == 'play storymode' then
        setPropertyFromClass('states.PlayState', 'isStoryMode', true)
        setPropertyFromClass('states.PlayState', 'storyPlaylist', {'opening', 'null-and-void', 'chkdsk', 'bloodlust', 'access-denied'})

        setVar('settingStory', true)
        openCustomSubstate('diffSelector')
    end

    if tag == 'play freeplay song' then
        setPropertyFromClass('states.PlayState', 'isStoryMode', false)
        ...
        elseif curSelectedFreeplay == 1 then
            loadSong('chkdsk', curDiff)
        elseif curSelectedFreeplay == 2 then
            loadSong('bloodlust', curDiff)
    ...
    if tag == 'start credits' then
        setDataFromSave('corruptMenu', 'main', false)
        setDataFromSave('corruptMenu', 'credits', true)
        restartSong()
    end

    if tag == 'play if' then
        setDataFromSave('corruptMenu', 'if', true)
        setDataFromSave('corruptMenu', 'main', false)
        restartSong()
    end
    ...
    if tag == 'cover black' then
        setProperty('blackCovering.alpha', 1)
    end
end
